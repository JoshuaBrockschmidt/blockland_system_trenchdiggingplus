	...
	if (%dbName $= "brick8x16DirtData") {
		...
		if (BTT_AABBAABB_2D(%brickAABB, %boxAABB)) {
			if (%dbName !$= "brick1x1DirtData") {
				...
				if (0) { // COMMENT
					%displace1 = vectorScale("0.25 0.25 0", %newDbName.brickSizeX);
					%displace2 = vectorScale("-0.25 0.25 0", %newDbName.brickSizeX);
					%displace3 = vectorScale("0.25 -0.25 0", %newDbName.brickSizeX);
					%displace4 = vectorScale("-0.25 -0.25 0", %newDbName.brickSizeX);
					%newPos1 = vectorAdd(%brickPos, %displace1);
					%newPos2 = vectorAdd(%brickPos, %displace2);
					%newPos3 = vectorAdd(%brickPos, %displace3);
					%newPos4 = vectorAdd(%brickPos, %displace4);
